...
document.getElementById('extractHours').addEventListener('click', async () => {
  try {
    ...
    // Format dates and display results
    const formattedResults = formatDates(result[0].result);
    displayResults(formattedResults);
    
    // Store the extracted data
    chrome.storage.local.set({ extractedHours: formattedResults });
...
function formatDates(data) {
  return data.map(dayData => {
    // Remove Hebrew characters from the date string
    const cleanDate = dayData.date.replace(/[א-ת]/g, '').trim();
    
    // Create a date object
    const dateObj = new Date(parseInt(dayData.year), new Date().getMonth(), parseInt(cleanDate));
    
    const year = dateObj.getFullYear();
    const month = String(dateObj.getMonth() + 1).padStart(2, '0');
    const day = String(dateObj.getDate()).padStart(2, '0');
    
    const formattedDate = `d${year}-${month}-${day}`;
...
function displayResults(data) {
  ...
  table.innerHTML = `
    <tr>
      <th colspan="5" style="text-align: center; background-color: #f8f9fa;">Year: ${year}</th>
    </tr>
    <tr>
      <th>Date</th>
...
function injectHoursToHRPortal(hoursData) {
  ...
  async function processDay(dayData) {
      ...
      try {
          ...
          const dayElement = document.querySelector(`div.cv-day[class*="${dayData.formattedDate}"]`);

          if (!dayElement) {
              console.error(`Day element not found for date: ${dayData.formattedDate}`);

              // Attempt to find the element using regex
              const regex = new RegExp(`d\\d{4}-\\d{2}-\\d{2}.*`);
